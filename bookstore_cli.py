...
def welcome_screen() -> None:
    ...
    print("Browse and manage your music bookstore database.")
    pause()
...
def view_categories(cursor: sqlite3.Cursor) -> None:
    ...
    if rows:
        for category_id, name, image in rows:
            print(f"ID: {category_id} | Name: {name} | Image: {image}")
    else:
...
def view_books(cursor: sqlite3.Cursor) -> None:
    cursor.execute(
        """
        SELECT book.bookId, book.title, book.author, category.categoryName, book.price, book.readNow
        FROM book
    ...
    if rows:
        for book_id, title, author, category_name, price, read_now in rows:
            status = "Yes" if read_now == 1 else "No"
            print(
                f"ID: {book_id} | Title: {title} | Author: {author} | "
                f"Category: {category_name} | Price: ${price:.2f} | Read Now: {status}"
            )
    else:
...
def view_books_in_category(cursor: sqlite3.Cursor) -> None:
    ...
    if rows:
        for book_id, title, author, category_name, price in rows:
            print(
                f"ID: {book_id} | Title: {title} | Author: {author} | "
                f"Category: {category_name} | Price: ${price:.2f}"
            )
    else:
...
def search_by_title(cursor: sqlite3.Cursor) -> None:
    ...
    if rows:
        for book_id, title, author, price in rows:
            print(f"ID: {book_id} | Title: {title} | Author: {author} | Price: ${price:.2f}")
    else:
        print("No books found.")
...
def search_by_author(cursor: sqlite3.Cursor) -> None:
    ...
    print_divider()
    print("Books by matching author")
